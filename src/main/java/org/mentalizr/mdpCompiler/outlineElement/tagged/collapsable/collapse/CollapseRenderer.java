...
public class CollapseRenderer extends OutlineElementRenderer {
    ...
    private void renderCollapse(int index, CollapsableAttributes collapsableAttributes, int indent, String id, CollapsableCardContent collapsableCardContent, Result result) throws MDPSyntaxError {
//        int collapsableIndex = collapsableCardContent.getIndex();
        String elementId = id + "_collapse_" + index;

        result.addLn(indent + 1, "<div class=\"collapse" + this.getShowFirst(collapsableAttributes, index) + "\" id=\"" + elementId + "\" data-parent=\"#" + id + "\">");
