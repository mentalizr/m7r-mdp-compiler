...
public class AccordionRenderer extends OutlineElementRenderer {
    ...
    public void render(OutlineElementModel outlineElementModel, CompilerContext compilerContext, Result result) throws MDPSyntaxError {
        ...
        for (int i = 0; i < collapsableModel.getCollapsableCardContentList().size(); i++) {

            result.addLn(indent + 1, "<div class=\"card\">");

            createAccordionCardHeader(collapsableModel, id, i, indent, result);
            createCardBody(collapsableModel, collapsableAttributes, id, i, indent, result);
    ...
    private void createAccordionCardHeader(CollapsableModel collapsableModel, String id, int cardIndex, int indent, Result result) {
    ...
    private void createCardBody(CollapsableModel collapsableModel, CollapsableAttributes collapsableAttributes, String id, int cardIndex, int indent, Result result) throws MDPSyntaxError {
        String cardHeaderId = getCardHeaderId(id, cardIndex);
        String cardCollapseId = getCardCollapseId(id, cardIndex);
        CollapsableCardContent collapsableCardContent = collapsableModel.getCollapsableCardContentList().get(cardIndex);
